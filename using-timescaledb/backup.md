...
>use it if you first restore the `_timescaledb_catalog` schema without
>the `-j` option, for example:
>```
>pg_restore -Fc -n _timescaledb_catalog -d tutorial tutorial.bak
>```
>And then restore the rest of the database with the `-j` option using
> the `-N` option, for example:
>```
>pg_restore -Fc -N _timescaledb_catalog -j4 -d tutorial tutorial.bak
>```
